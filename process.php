...
function addCons(){
	...
	$sql = "INSERT INTO ups_shipped_item (cons_no, ship_name, phone, s_add, rev_name, r_phone, r_add,  type, weight, invice_no, qty, book_mode, freight, mode, pick_date, pick_time, status, comments, book_date )
...
function addNewOffice() {
	...
	$sql = "INSERT INTO ups_retail_center (off_name, address, city, ph_no, office_time, contact_person)
...
function addManager() {
	...
	$sql = "INSERT INTO ups_retail_center_man(officer_name, off_pwd, address, email, ph_no, office, reg_date)
...
function updateStatus() {
	...
	$sql = "INSERT INTO ups_shipped_item_track (cid, cons_no, current_city, status, comments, bk_time)
			VALUES ($cid, '$cons_no', '$OfficeName', '$status', '$comments', NOW())";
	dbQuery($sql);
	
	$sql_1 = "UPDATE ups_shipped_item 
				SET status = '$status' 
				WHERE cid = $cid
				AND cons_no = '$cons_no'";
	dbQuery($sql_1);	
